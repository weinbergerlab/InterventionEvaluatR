...
```{r univariate, echo = T, results = 'hide'}
 glmer_results= evaluatr.univariate(analysis)
```

Then plot the results. The results are ordered by goodness of fit (based on AIC scores), with best fitting covariates on top. Each plot represents a different age group. 

```{r univariate_plot,fig.width=4, fig.height=5}
par(mar=c(2,5,1,1)) #fix margins
group.labels<-unique(analysis$input_data[,analysis$group_name])
...
impact_results$best$rr_mean[,c(2,1,3)]
