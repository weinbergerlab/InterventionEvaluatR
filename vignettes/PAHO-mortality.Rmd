...
These models are fit using Markov Chain Monte Carlo (MCMC). It is important to evaluate the convergence of the model. A quick way to check this is to evaluate the trace plots for rate ratio estimates from the synthetic controls model (or for the other model variants). If the model has not converged, you might need to add more iterations or a longer burn in period
```{r}
trace1<-impact_results$full$rr_iter
for(i in 1: nrow(trace1)){
  n.iter<-length(trace1[i,])
  samp1<-trace1[i,1:round(0.1*n.iter)] #1st 10%
  samp2<-trace1[i,-c(1:round(0.05*n.iter))] #last 50%
  geweke.p<- t.test(samp1, samp2)$p.value
  print(geweke.p)
if(geweke.p>0.05){con.stat<-'Model converged'
  }else{
    con.stat<-'Not converged'
  } 
   plot(trace1[i,], type='l', main=paste0(con.stat,' ',colnames(trace1)[i]), bty='l')
  }
```
