...
#DIC
#This function calculate the -2*log likelihood for each column D
#y are the observed counts

#Need to calculate pred count mean by calculating mean of each of the betas and RE, then calculate pred count from that
pre.index<- which( unique(analysis$input_data[, analysis$date_name])< analysis$intervention_date)

post.mat<-impact_results$full$groups$`ec 2-23m A`$reg.mean #lambda
post.mat.pre<-post.mat[pre.index,]
obs<-impact_results$full$groups$`ec 2-23m A`$observed.y

pred.count<- post.mat.pre #lambda
pred.count.mean<-apply(pred.count,1,mean)
obs.pre<-obs[pre.index]
...
DIC2<- 2*(mean(log.lik.mat)) -   neg_two_loglike_poisson_mean
