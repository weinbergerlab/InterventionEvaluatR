...
call.jags.mod<-function(ds.in){
  ...
  ds2.pre<-ds2[ds1$date<analysis$post_period[1] ,]
  ds3<-ds2
  ds3[,analysis$outcome_name][ds1$date>=analysis$post_period[1]]<-NA
  ...
  month.mat.pre<- month.mat[ds1$date<analysis$post_period[1] ,]
  
  mod.txt<- source('../jags/Non_spatial-Non_lagged-independent.txt')
  mod1<-nonspace_nonlag(burnin=20000, 
                        samples=20000,
                        thin=1,
                        chains=1,
                        regularize=TRUE, 
  ...
  # 
    log.rr.pointwise<- log((ds1[(nrow(ds2.pre)+1):nrow(ds1),analysis$outcome_name]+0.5)/(post.combo.y+0.5))
   ...
   obs.post.sum<- sum(ds2$sALRI[ds1$date>=analysis$post_period[1]][-c(1:length.rollout)])
   rr.agg<-obs.post.sum/post.samples.sum
   rr.q<-quantile(rr, probs=c(0.025, 0.5, 0.975))
...
mod1<-lapply(ds.age.spl,call.jags.mod)
```
