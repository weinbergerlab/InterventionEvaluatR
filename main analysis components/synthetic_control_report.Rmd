...
kable( cbind.data.frame(rr_mean_full_intervals,rr_mean_time_intervals, rr_mean_pca_intervals), align = 'c')
```

#Plot of Rate ratios, with size proportional to WAIC weights
```{r fig.width=5, fig.height=3, fig.align = "center", dpi=300, echo=FALSE}
#Compare rate ratios, with size of marker scaled to WAIC weights
	  ...
	  geom_point(aes(shape=Model, size=index))+
	  scale_size_manual(values=c(waic_weights.m$value*2)) + #Scales area, which is optimal for bubbl plot
...
##WAIC scores for each of the models
```{r Comparison of WAIC from different models}
kable(waic_combo)
```
