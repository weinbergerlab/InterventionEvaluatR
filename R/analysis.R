syncon.init <- function(
  ...
  analysis$n_seasons <- n_seasons #Number of months (seasons) per year. 12 for monthly, 4 for quarterly, 3 for trimester data.
  analysis$year_def <- year_def #Can be cal_year to aggregate results by Jan-Dec; 'epi_year' to aggregate July-June

  #MOST DATES MUST BE IN FORMAT "YYYY-MM-01", exception is end of pre period, which is 1 day before end of post period
  analysis$pre_period <- as.Date(c(pre_period_start, pre_period_end)) #Range over which the data is trained for the CausalImpact model.
  analysis$start_date <- analysis$pre_period[1]
  analysis$post_period <- as.Date(c(post_period_start, post_period_end )) #Range from the intervention date to the end date.
  analysis$intervention_date <- analysis$post_period[1]-1
  analysis$end_date <- analysis$post_period[2]

  analysis$eval_period <- as.Date(c(eval_period_start, eval_period_end)) #Range over which rate ratio calculation will be performed.

  analysis$group_name <- group_name #Name of column containing group labels.
  analysis$date_name <- date_name #Name of column containing dates.
  analysis$outcome_name <- outcome_name #Name of column containing outcome.
  analysis$denom_name <- denom_name #Name of column containing denominator to be used in offset.

  analysis$.private$exclude_covar <- c() #User-defined list of covariate columns to exclude from all analyses.
  analysis$.private$exclude_group <- c() #User-defined list of groups to exclude from analyses.
...
syncon.impact = function(analysis) {
  ...
  for (variant in names(analysis$.private$variants)) {
    results[[variant]]$groups <- setNames(pblapply(
      cl=cl, analysis$.private$data[[variant]], FUN=doCausalImpact, 
  ...
  for (variant in c('full', 'time')) {
    ...
    results[[variant]]$inclusion_prob <- setNames(lapply(results[[variant]]$groups, inclusionProb), analysis$groups)
  }

  for (variant in names(analysis$.private$variants)) {
    #All model results combined
    results[[variant]]$quantiles <- setNames(lapply(analysis$groups, FUN=function(group) {
      rrPredQuantiles(impact = results[[variant]]$groups[[group]], denom_data = analysis$.private$ds[[group]][, analysis$denom_name], eval_period=analysis$eval_period, post_period=analysis$post_period, year_def=analysis$year_def, time_points=analysis$time_points, n_seasons=analysis$n_seasons)
    }), analysis$groups)
  }

  # Calculate best model
  analysis$model_size <- sapply(results$full$groups, modelsize_func, n_seasons=analysis$n_seasons)
  results$best$quantiles <- vector("list", length(results$full$quantiles)) 
  results$best$quantiles[analysis$model_size>=1] <- results$full$quantiles[analysis$model_size>=1]
  results$best$quantiles[analysis$model_size<1] <- results$pca$quantiles[analysis$model_size<1]
  results$best$quantiles <- setNames(results$best$quantiles, analysis$groups)
  ...
  for (variant in c('full', 'best')) {
    ...
    dimnames(results[[variant]]$log_rr_quantiles)[[1]] <- analysis$time_points
  ...
  for (variant in c("best", names(analysis$.private$variants))) {
    ...
    ), check.names = FALSE, row.names = analysis$groups), c(paste(analysis$.private$variants[[variant]]$name, 'Estimate (95% CI)')))
  }

  colnames(results$time$rr_mean) <- paste('Time_trend', colnames(results$time$rr_mean))

  for (variant in c("best", names(analysis$.private$variants))) {
    results[[variant]]$cumsum_prevented <- sapply(analysis$groups, FUN=cumsum_func, quantiles = results[[variant]]$quantiles, outcome=analysis$outcome, analysis$time_points, analysis$post_period, simplify = 'array')
  }

  #Run a classic ITS analysis
  rr.its1 <- lapply(analysis$.private$data$time, its_func, post_period=analysis$post_period, eval_period=analysis$eval_period, time_points=analysis$time_points)
  rr.t <- sapply(rr.its1, `[[`, "rr.q.t", simplify='array')
  results$its = list()
  results$its$rr_end <- t(sapply(rr.its1, `[[`, "rr.q.post", simplify='array')) 
  results$its$rr_mean_intervals <- data.frame('Classic ITS (95% CI)' = makeInterval(results$its$rr_end[, 2], results$its$rr_end[, 3], results$its$rr_end[, 1]), check.names = FALSE, row.names = analysis$groups)

  #Combine RRs into 1 for ease of plotting
  results$rr_mean_combo<- as.data.frame(rbind(
    cbind(rep(1, nrow(results$full$rr_mean)), analysis$groups, seq(from=1, by=1, length.out=nrow(results$full$rr_mean)), results$full$rr_mean),
    cbind(rep(2, nrow(results$time$rr_mean)), analysis$groups, seq(from=1, by=1, length.out=nrow(results$time$rr_mean)), results$time$rr_mean),
    cbind(rep(3, nrow(results$time_no_offset$rr_mean)), analysis$groups, seq(from=1, by=1, length.out=nrow(results$time_no_offset$rr_mean)), results$time_no_offset$rr_mean),
    cbind(rep(4, nrow(results$pca$rr_mean)), analysis$groups, seq(from=1, by=1, length.out=nrow(results$pca$rr_mean)), results$pca$rr_mean)
  ))
...
syncon.crossval = function(analysis) {
  ...
  for (variant in names(analysis$.private$variants)) {
    analysis$.private$data.cv[[variant]]<-lapply(analysis$.private$data[[variant]], makeCV, analysis$time_points, analysis$intervention_date)
  }
  ...
  for (variant in names(analysis$.private$variants)) {
    results[[variant]]$groups <-setNames(pblapply(
      cl=cl, analysis$.private$data.cv[[variant]], FUN=function(x) lapply(
        x, doCausalImpact, 
        analysis$intervention_date, 
        analysis$n_seasons,
        time_points=analysis$time_points,
  ...
  names(results$stacking_weights.all) <- lapply(analysis$.private$variants, function(v) { v$name })
  results$stacking_weights.all <- cbind.data.frame(data.frame(groups=analysis$groups), results$stacking_weights.all)
  ...
  stacked.ests <- mapply(
    ...
    MoreArgs=list(stacking_weights.all=results$stacking_weights.all, outcome=analysis$outcome), 
    SIMPLIFY=FALSE
  )
  # plot.stacked.ests<-lapply(stacked.ests,plot.stack.est)
  results$quantiles_stack <- setNames(lapply(analysis$groups, FUN = function(group) {
    rrPredQuantiles(impact = stacked.ests[[group]], denom_data = analysis$.private$ds[[group]][, analysis$denom_name], analysis$eval_period, analysis$post_period, analysis$n_seasons, analysis$year_def, analysis$time_points)
  }), analysis$groups)
  ...
  results$rr_mean_stack_intervals <- data.frame('Stacking Estimate (95% CI)' = makeInterval(results$rr_mean_stack[, 2], results$rr_mean_stack[, 3], results$rr_mean_stack[, 1]), check.names = FALSE, row.names = analysis$groups)
  results$cumsum_prevented_stack <- sapply(analysis$groups, FUN = cumsum_func, quantiles = results$quantiles_stack, outcome=analysis$outcome, analysis$time_points, analysis$post_period, simplify = 'array')
...
syncon.sensitivity = function(analysis) {
  results = list()
  bad_sensitivity_groups <- sapply(analysis$covars$full, function (covar) {ncol(covar) <= analysis$n_seasons-1+3})
  sensitivity_covars_full <- analysis$covars$full[!bad_sensitivity_groups]
  sensitivity_ds <- analysis$.private$ds[!bad_sensitivity_groups]
  sensitivity_impact_full <- analysis$results$impact$full$groups[!bad_sensitivity_groups]
  ...
  if (length(sensitivity_groups)!=0) {
    ...
    sensitivity_analysis_full <- setNames(pblapply(cl=cl, sensitivity_groups, FUN=weightSensitivityAnalysis, covars = sensitivity_covars_full, ds = sensitivity_ds, impact = sensitivity_impact_full, time_points = analysis$time_points, intervention_date = analysis$intervention_date, n_seasons = analysis$n_seasons, outcome = analysis$outcome, eval_period = analysis$eval_period, post_period = analysis$post_period, year_def=analysis$year_def), sensitivity_groups)
    stopCluster(cl)
    ...
    results$sensitivity_table <- t(sapply(sensitivity_groups, sensitivityTable, sensitivity_analysis = sensitivity_analysis_full, original_rr = analysis$results$impact$full$rr_mean))
    ...
    results$rr_table <- cbind.data.frame(round(analysis$results$impact$time$rr_mean[!bad_sensitivity_groups, ],2), results$sensitivity_table)
    results$rr_table_intervals <- cbind('Trend Estimate (95% CI)' = analysis$results$impact$time$rr_mean_intervals[!bad_sensitivity_groups, ], results$sensitivity_table_intervals)
  } else {
...
syncon.impact.pre = function(analysis) {
  ...
  prelog_data[, analysis$date_name] <- formatDate(prelog_data[, analysis$date_name])
  prelog_data <- setNames(lapply(
    analysis$groups, FUN=splitGroup, 
    ungrouped_data=prelog_data, group_name=analysis$group_name, date_name=analysis$date_name, 
    start_date=analysis$start_date, end_date=analysis$end_date, 
    no_filter=c(analysis$group_name, analysis$date_name, analysis$outcome_name, analysis$denom_name)
  ), analysis$groups)
  ...
  analysis$.private$ds <- setNames(lapply(prelog_data, FUN = logTransform, no_log = c(analysis$group_name, analysis$date_name, analysis$outcome_name)), analysis$groups)
  analysis$time_points <- unique(analysis$.private$ds[[1]][, analysis$date_name])
  ...
  analysis$.private$ds <- lapply(analysis$.private$ds, function(ds) {
  ...
  analysis$sparse_groups <- sapply(analysis$.private$ds, function(ds) {
    return(ncol(ds[!(colnames(ds) %in% c(analysis$date_name, analysis$group_name, analysis$denom_name, analysis$outcome_name, analysis$.private$exclude_covar))]) == 0)
  })
  analysis$.private$ds <- analysis$.private$ds[!analysis$sparse_groups]
  ...
  analysis$covars$full <- setNames(lapply(analysis$.private$ds, function(group) { makeCovars(analysis$country, analysis$time_points, analysis$intervention_date, season.dummies, group) }), analysis$groups)
  analysis$covars$full <- lapply(analysis$covars$full, FUN = function(covars) {covars[, !(colnames(covars) %in% analysis$.private$exclude_covar), drop = FALSE]})
  analysis$covars$time <- setNames(lapply(analysis$covars$full, FUN = function(covars) {as.data.frame(list(cbind(season.dummies,time_index = 1:nrow(covars))))}), analysis$groups)
  analysis$covars$null <- setNames(lapply(analysis$covars$full, FUN = function(covars) {as.data.frame(list(cbind(season.dummies)))}), analysis$groups)

  #Standardize the outcome variable and save the original mean and SD for later analysis.
  analysis$outcome <- sapply(analysis$.private$ds, FUN = function(data) {data[, analysis$outcome_name]})
  offset <- sapply(analysis$.private$ds, FUN=function(data) exp(data[, analysis$denom_name]) ) #offset term on original scale; 1 column per age group
  ...
  stl.covars<-mapply(smooth_func,ds.list=analysis$.private$ds,covar.list=analysis$covars$full, SIMPLIFY=FALSE, MoreArgs=list(n_seasons=analysis$n_seasons)) 
  post.start.index<-which(analysis$time_points==analysis$post_period[1])

  if (length(analysis$groups)>1){ 
    stl.data.setup<-mapply(stl_data_fun,covars=stl.covars, ds.sub=analysis$.private$ds ,SIMPLIFY=FALSE, MoreArgs=list(n_seasons=analysis$n_seasons, outcome_name=analysis$outcome_name, post.start.index=post.start.index)) #list of lists that has covariates for each regression for each strata
  }else{
    stl.data.setup <- list(mapply(stl_data_fun,covars=stl.covars, ds.sub=analysis$.private$ds, MoreArgs=list(n_seasons=analysis$n_seasons, outcome_name=analysis$outcome_name, post.start.index=post.start.index)))
  }
  ...
  analysis$.private$data$full <- setNames(lapply(analysis$groups, makeTimeSeries, outcome=analysis$outcome, covars=analysis$covars$full), analysis$groups)
  analysis$.private$data$time <- setNames(lapply(analysis$groups, makeTimeSeries, outcome=analysis$outcome, covars=analysis$covars$time, trend=TRUE, offset=offset), analysis$groups)
  analysis$.private$data$pca <- mapply(FUN=pca_top_var, glm.results.in=glm.results, covars=stl.covars, ds.in=analysis$.private$ds, SIMPLIFY=FALSE, MoreArgs=list(outcome_name=analysis$outcome_name, season.dummies=season.dummies))
  names(analysis$.private$data$pca) <- analysis$groups
  #Time trend model but without a denominator
  analysis$.private$data$time_no_offset <- setNames(lapply(analysis$groups, makeTimeSeries, outcome=analysis$outcome, covars=analysis$covars$time, trend=FALSE), analysis$groups)
}
