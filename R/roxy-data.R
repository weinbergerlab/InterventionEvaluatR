#' TODO
#' 
#' @name pnas_brazil
#' @docType data
#' @format Monthly time series of cause-specific hospitalizations from Brazil, as described in Bruhn et al., PNAS. 
#' The columns refer to sets of ICD10 codes. 
#' 'age_group' categories: 
#'   0: <3months 
#'   1:3-11m
#'   2:12-23m 
#'   3:24-59m 
#'   4:5-17 years
#'   5: 18-39 years
#'   6:40-64 years
#'   7:65-79 years
#'   8: 80+ years
#'   9: <12 months
NULL

#' TODO
#' 
#' @name pnas_chile
#' @docType data
#' @format Monthly time series of cause-specific hospitalizations from Chile, as described in Bruhn et al., PNAS. 
#' The columns refer to sets of ICD10 codes. 
#' 'age_group' categories: 
#'   2:12-23m 
#'   3:24-59m 
#'   4:5-17 years
#'   5: 18-39 years
#'   6:40-64 years
#'   7:65-79 years
#'   8: 80+ years
#'   92: <24 months
NULL

#' TODO
#' 
#' @name pnas_ecuador
#' @docType data
#' @format Monthly time series of cause-specific hospitalizations from Ecuador, as described in Bruhn et al., PNAS. 
#' The columns refer to sets of ICD10 codes. 
#' 'age_group' categories: 
#'   0: <3months 
#'   1:3-11m
#'   2:12-23m 
#'   3:24-59m 
#'   4:5-17 years
#'   5: 18-39 years
#'   6:40-64 years
#'   7:65-79 years
#'   8: 80+ years
#'   9: <12 months
NULL

#' TODO
#' 
#' @name pnas_mexico
#' @docType data
#' @format Monthly time series of cause-specific hospitalizations from Mexico, as described in Bruhn et al., PNAS. 
#' The columns refer to sets of ICD10 codes. 
#' 'age_group' categories: 
#'   0: <3months 
#'   1:3-11m
#'   2:12-23m 
#'   3:24-59m 
#'   4:5-17 years
#'   5: 18-39 years
#'   6:40-64 years
#'   7:65-79 years
#'   8: 80+ years
#'   9: <12 months
NULL

#' TODO
#' 
#' @name pnas_us_ipd
#' @docType data
#' @format Monthly time series of cause-specific hospitalizations from Brazil, as described in Bruhn et al., PNAS. 
#' The potential outcome variables are: m_ACP (all-cause pneumonia); m_IPD: (invasive pneumococcal disease); or
#' gg_ACP: a more stringent definition of pneumonia (enriched for community-acquired pneumonia), as developed by Carlos Grijalva
#' and Marie Griffin. 'ach_sid_noresp'is all non-respiratory causes of hospitalization.
#' The columns refer to sets of ICD9 codes. Note small counts are masked with 99999.  
#' 'age_group' categories: 
#'   0: <3months 
#'   1: 3-11m
#'   2: 12-23m 
#'   3: 24-59m 
#'   4: 5-17 years
#'   5: 18-39 years
#'   6: 40-64 years
#'   7: 65-79 years
#'   8: 80+ years
#'   9: <12 months
NULL

#' TODO
#' 
#' @name pnas_us_pneumonia
#' @docType data
#' @format Monthly time series of cause-specific hospitalizations from Brazil, as described in Bruhn et al., PNAS. 
#' The columns refer to sets of ICD9 codes. 'ach_sid_noresp'is all non-respiratory causes of hospitalization.
#' Note small counts are masked with 99999.  
#' 'age_group' categories: 
#'   0: <3months 
#'   1: 3-11m
#'   2: 12-23m 
#'   3: 24-59m 
#'   4: 5-17 years
#'   5: 18-39 years
#'   6: 40-64 years
#'   7: 65-79 years
#'   8: 80+ years
#'   9: <12 months
NULL


#' TODO
#' 
#' @name ecuador_mortality
#' @docType data
#' @format Monthly time series of cause-specific deaths from Ecuador, as described in Oliveira and Shioda et al., 2019 
#' The columns refer to sets of ICD10 codes. "acm_noj_prim" is all deaths excluding those who have a
#' respiratory cause of death (J-chapter) anywhere in the record
#' 'age_group' categories: 
#'   ec 2-23m A: 2-23 months, National-level 
#'   ec 24-59m A: 24-59 months, National-level 
#'   ec 2-59m A: 2-59 months, National-level 
NULL
